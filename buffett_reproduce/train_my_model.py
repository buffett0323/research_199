...
# with torch.no_grad():
#     for mixture, query_emb, target, test_stem in test_loader:
#         mixture = mixture.to(device)
#         query_emb = query_emb.mean(axis=1).to(device)
#         target = target.to(device)

#         # Forward pass
#         pred_masks, gt_masks, gt_stft, S_k = model(mixture, query_emb, target)

#         # Compute the loss
#         loss = criterion(pred_masks, gt_masks, gt_stft, S_k)
#         test_loss += loss.item()

#         # # Calculate metrics
#         # sdr, sir, sar = cal_metrics(gt_stft, S_k)
#         # print("Test Metrics:\n", "test_sdr": sdr, "test_sir": sir, "test_sar": sar)
#         preds, target = S_k[:,0,:].to("cpu"), gt_stft[:,0,:].to("cpu")
#         test_metric_handler.calculate_snr(preds, target, test_stem)
